...
pub async fn start_info_listener(ctx: Arc<crate::nats_consumer::WorkerContext>) {
    let info_sub = ctx
        .nats_client
        .subscribe(subjects::INFO_REQUEST.to_string())
        .await;
    if let Ok(mut sub) = info_sub {
        ...
        tokio::spawn(async move {
            ...
            while let Some(msg) = sub.next().await {
                tracing::info!("Received InfoRequest message");
                if let Ok(req) = serde_json::from_slice::<fsocial_common::InfoRequest>(&msg.payload)
                {
                    tracing::info!("Parsed InfoRequest for URL: {}", req.url);
                    if let Some(reply) = msg.reply {
                        let res = if req.url.contains("spotify.com") {
                            let mut playlist_urls = Vec::new();
                            let is_playlist =
                                req.url.contains("/playlist/") || req.url.contains("/album/");
                            let mut title = if is_playlist {
                                "Spotify Плейлист/Альбом".to_string()
                            } else {
                                "Spotify Audio".to_string()
                            };
                            ...
                            if let Some((stype, sid)) =
                                audio::spotify::SpotifyClient::parse_spotify_url(&req.url)
                            {
                                if stype == audio::spotify::SpotifyType::Track {
                                    if let Ok(meta) =
                                        spotify_client.get_track(&ctx_info.config, &sid).await
                                    {
                            ...
                            if let Ok(resp) = client.get(&req.url).send().await {
                                if let Ok(html) = resp.text().await {
                                    ...
                                    if title == "Spotify Audio"
                                        || title == "Spotify Плейлист/Альбом"
                                    {
                                        if let Some(caps) = regex::Regex::new(
                                            ...
                                            if !is_playlist {
                                                if title.contains("·") {
                                                    ...
                                                    let parts: Vec<&str> = title
                                                        .split(" - song and lyrics by ")
                                                        .collect();
                                                    if parts.len() == 2 {
                                                        let new_title = parts[0].to_string();
                                                        let artist_part = parts[1]
                                                            .split(" | Spotify")
                                                            .next()
                                                            .unwrap_or(parts[1]);
                                    ...
                                    if is_playlist {
                                        if let Some((stype, sid)) =
                                            audio::spotify::SpotifyClient::parse_spotify_url(
                                                &req.url,
                                            )
                                        {
                                            let mut urls = vec![];
                                            if stype == audio::spotify::SpotifyType::Playlist {
                                                if let Ok(u) = spotify_client
                                                    .get_playlist_track_urls(&ctx_info.config, &sid)
                                                    .await
                                                {
                                                    urls = u;
                                                }
                                                ...
                                                if let Ok(u) = spotify_client
                                                    .get_album_track_urls(&ctx_info.config, &sid)
                                                    .await
                                                {
                                                    urls = u;
                                                }
                                            ...
                                            if urls.is_empty() {
                                                tracing::warn!(
                                                    "Spotify API returned 0 tracks. Falling back to yt-dlp..."
                                                );
                                                if let Ok(info) = media::ytdlp::get_info(
                                                    &ctx_info.config,
                                                    &req.url,
                                                    ctx_info.proxy_pool.next(),
                                                )
                                                .await
                                                {
                                            ...
                                            if playlist_urls.len() >= 30
                                                && ctx_info.config.spotify_client_id.is_none()
                                            {
                                                title = format!(
                                                    "{} (Показано 30 треков. Добавьте API ключи Spotify в .env для полных плейлистов)",
                                                    title
                                                );
                                            }
                                        }
                            ...
                            let mut qual_opt = fsocial_common::QualityOption {
                                ...
                                filesize_bytes,
                                estimated_secs: filesize_bytes
                                    .map(|b| (b / (10 * 1024 * 1024)).max(1)),
                            ...
                            let mut info_res = Err(fsocial_common::AppError::Download(
                                "Failed to get info".into(),
                            ));
                            ...
                            while info_attempts < max_info_attempts {
                                ...
                                if info_res.is_ok() {
                                    ...
                                    tracing::warn!(
                                        "get_info failed on url {} (attempt {}/{}). Retrying... Error: {:?}",
                                        req.url,
                                        info_attempts,
                                        max_info_attempts,
                                        e
